...
export default function Home({ serverAuth, serverPlaceCard, serverH2, serverContextState }) {
  ...
  const [auth, updateAuth] = useState(serverAuth);
  const [h2, updateH2] = useState(serverH2)
  const [placeCard, updatePlaceCard] = useState(serverPlaceCard);
...
console.log(ctx.resolvedUrl);
  ...
  if (session) {
    ...
    const url = ctx.req.headers.host + "/api/movies?listDate=" + ctx.query.id;
    console.log(url);
    const response = await fetch(url)
    const responseObj = await response.json();
    if (typeof responseObj[0] === 'string') {
      ...
      return {
        props: {
          serverAuth: true,
          serverContextState: responseObj.data,
          serverH2: responseObj.listTitle,
          serverPlaceCard: true,
        }
      }
    }
